...
pub fn build_ty(
    ...
    constants: &naga::Arena<naga::Constant>,
    ...
    match &ty.name {
        Some(name) => {
            ...
            let pre_pos = name.find(NAGA_OIL_DECORATION_PRE);
            let end_pos = name.find(NAGA_OIL_DECORATION_POST);

            let name = match (pre_pos, end_pos) {
                (Some(pre_pos), Some(end_pos)) if end_pos > pre_pos => {
                    let mut res = name[..pre_pos].to_string();
                    res.push_str(&name[end_pos + NAGA_OIL_DECORATION_POST.len()..]);
                    res
                }
                _ => name.clone(),
            };
        ...
        None => build_ty_inner(&ty.inner, types, constants, def_paths),
    }
}
...
pub fn build_ty_inner(
    ...
    let name =
        match *type_inner {
            ...
            Ti::Pointer { base, .. } => {
                let base = &types[base];
                return docs::Type::Pointer(Box::new(build_ty(base, types, constants, def_paths)));
            }
            ...
            Ti::Array { base, size, .. } => {
                ...
                return match size {
                    naga::ArraySize::Constant(size) => {
                        ...
                        let size = match &constant.inner {
                            naga::ConstantInner::Scalar { value, .. } => match *value {
                                naga::ScalarValue::Sint(v) => u32::try_from(v).ok(),
                                naga::ScalarValue::Uint(v) => u32::try_from(v).ok(),
                                naga::ScalarValue::Float(_) => None,
                                naga::ScalarValue::Bool(_) => None,
                            },
                            _ => None,
                        };

                        docs::Type::ArrayConstant(
                            Box::new(build_ty(member_type, types, constants, def_paths)),
                            size,
                        )
                    }
                    naga::ArraySize::Dynamic => docs::Type::ArrayDynamic(Box::new(build_ty(
                        member_type,
                        types,
                        constants,
                        def_paths,
            ...
            Ti::BindingArray { base, size, .. } => {
                ...
                return match size {
                    naga::ArraySize::Constant(size) => {
                        ...
                        docs::Type::BindingArrayConstant(
                            Box::new(build_ty(member_type, types, constants, def_paths)),
                            size,
                        )
                    }
                    naga::ArraySize::Dynamic => docs::Type::BindingArrayDynamic(Box::new(
                        build_ty(member_type, types, constants, def_paths),
                    )),
                };
            }
        };
